import { type LifelineResult } from '../types';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-md w-full p-6 relative shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
                ...
                <div className="text-center">
                    {result.type === 'guests' && (
                        <>
                            <div className="inline-block p-4 bg-purple-900/30 rounded-full mb-4 ring-4 ring-purple-900/20">
                                <ShieldCheck size={48} className="text-purple-400" />
                            </div>
                            <h3 className="text-2xl font-bold text-purple-400 mb-2">O Tech Lead diz...</h3>
                            <div className="bg-slate-950 p-6 rounded-xl border border-slate-800 mb-4 relative">
                                ...
                                <p className="text-lg text-gray-200 italic">
                                    "Analisei a documentação e os requisitos. Tenho 99% de certeza que a resposta é a opção <strong className="text-purple-400 text-xl">{getLabel(result.suggestion!)}</strong>."
                                </p>
                            </div>
                        </>
                    )}
                    ...
                    {result.type === 'placa' && (
                        <>
                            <div className="inline-block p-4 bg-blue-900/30 rounded-full mb-4 ring-4 ring-blue-900/20">
                                <Vote size={48} className="text-blue-400" />
                            </div>
                            <h3 className="text-2xl font-bold text-blue-400 mb-6">Resultado do Planning Poker</h3>
                    ...
                    <button
                        onClick={onClose}
                        className="mt-6 w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-lg transition-colors border border-slate-700"
                    >
                        Entendido
                    </button>
                </div>
