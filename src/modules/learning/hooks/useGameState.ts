...
import { type GameState, type Question, type Difficulty, type LifelineType, type TrilingualQuestion } from '../types';
...
export const useGameState = () => {
    ...
    const [activeQuestion, setActiveQuestion] = useState<TrilingualQuestion | null>(null);

    // Derived Current Question for UI
    const currentQuestion = useMemo((): Question | null => {
        if (!activeQuestion) return null;
        const langData = activeQuestion[language];
        return {
            id: activeQuestion.id.toString(), // Ensure string ID
            difficulty: activeQuestion.difficulty,
            correctAnswer: activeQuestion.correctOptionIndex.toString(), // Map index to string ID for now
            options: langData.options.map((opt, idx) => ({ id: idx.toString(), text: opt })), // Map strings to objects
            text: langData.text,
            language: language,
            correctDetails: langData.correctDetails
        };
    }, [activeQuestion, language]);

    // DB Questions Cache
    const [dbQuestions, setDbQuestions] = useState<TrilingualQuestion[]>([]);
        ...
        const fetchQuestions = async () => {
            ...
            try {
                ...
                if (data && data.length > 0) {
                    const mappedQuestions: TrilingualQuestion[] = data.map((q: any) => ({
                        id: q.id,
                        difficulty: q.difficulty,
                        correctOptionIndex: q.correct_option_index,
                        pt: { text: q.text_pt || q.text, options: q.options_pt || q.options, correctDetails: q.correct_details_pt || q.correct_details },
                        en: { text: q.text_en || q.text, options: q.options_en || q.options, correctDetails: q.correct_details_en || q.correct_details },
                        es: { text: q.text_es || q.text, options: q.options_es || q.options, correctDetails: q.correct_details_es || q.correct_details },
                    }));
                    setDbQuestions(mappedQuestions);
                    console.log("Loaded questions from Supabase:", mappedQuestions.length);
                } else {
                    console.warn("No questions found in DB, using local fallback.");
                    setDbQuestions(mapLocalQuestions(LOCAL_QUESTIONS));
                }

            } catch (err) {
                console.warn("Falling back to local questions due to error.", err);
                setDbQuestions(mapLocalQuestions(LOCAL_QUESTIONS));
            } finally {
                setLoading(false);
            }
    ...
    // Helper to map old local questions to Trilingual format
    const mapLocalQuestions = (local: any[]): TrilingualQuestion[] => {
        return local.map(q => ({
            id: q.id,
            difficulty: q.difficulty,
            correctOptionIndex: q.correctOptionIndex,
            pt: { text: q.text, options: q.options, correctDetails: q.correctDetails || '' },
            en: { text: q.text, options: q.options, correctDetails: q.correctDetails || '' },
            es: { text: q.text, options: q.options, correctDetails: q.correctDetails || '' },
        }));
    };

    // Helper to get questions from the LOADED pool (DB or Local)
    const getQuestionForLevel = useCallback((level: number, usedIds: (string | number)[]): TrilingualQuestion | null => {
        ...
        if (candidates.length === 0) {
            ...
            // Better fallback logic to avoid repeats if possible, but safe game loop essential
    ...
    // Timer Logic
    useEffect(() => {
        if (gameState.status !== 'playing' || !activeQuestion) return;
    ...
    useEffect(() => {
        if (!loading && activeQuestion === null && gameState.status === 'playing') {
            startGame();
        }
    }, [loading, startGame, activeQuestion, gameState.status]);

    // Handle Answer
    const handleAnswer = (optionIndex: number) => {
        if (!activeQuestion || gameState.status !== 'playing') return;

        const isCorrect = optionIndex === activeQuestion.correctOptionIndex;
    ...
    const useLifeline = (type: LifelineType) => {
        ...
        if (type === 'skip') {
            ...
            const wrongIndices = [0, 1, 2, 3].filter(i => i !== activeQuestion.correctOptionIndex);
            ...
            const correct = activeQuestion.correctOptionIndex;
            let suggestion = correct;

            // Error chance: Easy 10%, Medium 30%, Hard 60%
            let errorChance = 0.1;
            if (activeQuestion.difficulty === 'medium') errorChance = 0.3;
            if (activeQuestion.difficulty === 'hard') errorChance = 0.6;
            if (activeQuestion.difficulty === 'million') errorChance = 0.8;
            ...
            if (activeQuestion.difficulty === 'medium') confidence = 0.5;
            if (activeQuestion.difficulty === 'hard') confidence = 0.3; // Confused church
    ...
    return {
        gameState,
        currentQuestion, // This is now the derived one with correct language
        handleAnswer,
        handleStop,
        useLifeline,
        closeLifelineModal,
